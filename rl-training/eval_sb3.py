...
ap.add_argument("-a", "--algorithm", required=True, help="Which algorithm to run, one of (SAC, TQC)", type=str)
...
if baseline_run:
    ...
    if algo == "SAC":
        agent = SAC.load(restoredir)
    elif algo == "TQC":
        agent = TQC.load(restoredir)
    else:
        print(f'Algorithm must be one of (SAC, TQC), got {algo}')
        os._exit(1)
